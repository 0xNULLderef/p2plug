...
#define CON_COMMAND_F_COMPLETION(name, description) \
	void name##_callback(const CCommand& args); \
...
#define DETOUR_COMMAND(name) void name##_callback_hook(const CCommand& args)

#define DECL_COMMAND_COMPLETION(command) \
	DECL_DECLARE_AUTOCOMPLETION_FUNCTION(command) { \
		const char* cmd = #command " "; \
		char* match = (char*)partial; \
		if(std::strstr(partial, cmd) == partial) match = match + std::strlen(cmd); \
	...
	DECL_DECLARE_AUTOCOMPLETION_FUNCTION(command) { \
		...
		for (auto& item : list) { \
			...
			if (std::strlen(match) != std::strlen(cmd)) { \
				if (std::strstr(item.c_str(), match)) items.push_back(item); \
			} else items.push_back(item); \
		} \
		auto count = 0; \
		for (auto& item : items) std::strcpy(commands[count++], (std::string(cmd) + item).c_str()); \
		return count; \
	}
