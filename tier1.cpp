	...
	if(this->g_pCVar) {
		...
		if(listdemo) {
			...
			if(listdemo->m_fnCompletionCallback) {
				auto callback = (uintptr_t)listdemo->m_fnCommandCallback + Offsets::AutoCompletionFunc;
		...
		if(sv_lan) {
			...
			auto vtable = &this->ConVar_VTable;
		...
		this->g_pCVar->Original<_RemoveGlobalChangeCallback>(Offsets::RemoveGlobalChangeCallback);
	}
